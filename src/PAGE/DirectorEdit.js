...
import { ActorADD, DirectorADD } from "../component/ADD";
...
function DirectorEdit() {
    ...
    const handleNewActor = async(actor) => {
        ...
        try {
            const response = await axios.put(`http://localhost:8080/director/edit/${id}`, actor);
    ...
    /*useEffect(() => {
        const updateActor = async () => {
            try {
                const response = await axios.post(`http://localhost:8080/director/edit/${id}`, editActor);
                console.log("edit update data", response.data);
                setData(response.data); 
            } catch (err) {
                console.error('Error updating actor:', err);
    ...
    return (
        <div className="ActorEdit">
            <s/>
            <DirectorADD NewDirector={handleNewActor} data={data} />
        </div>
    );
}
